...
from customAddedConv import customAddedConvModel
from tag_compound import tag_compound
from tag_compound import train as train_tag
from tag_compound import inference as infer_tag
...
def run_mrc(data_args, training_args, model_args, datasets, tokenizer, model):
    ...
    if training_args.do_train:
        ...
        tag_train = pd.read_csv("./tag_compound/data/tag_train.tsv", header=None, sep="\t")[0]

        def add_tag_train(example, idx):
            example['question'] = tag_train[idx] + example['question']
            return example
    ...
    if training_args.do_eval:
        ...
        tag_valid = pd.read_csv("./tag_compound/data/tag_valid.tsv", header=None, sep="\t")[0]

        def add_tag_train(example, idx):
            example['question'] = tag_valid[idx] + example['question']
            return example

        eval_dataset = eval_dataset.map(add_tag_train, with_indices=True)
...
if __name__ == "__main__":
    ...
    if all(os.path.exists(os.path.join('./tag_compound/data', f)) for f in file_tag):
